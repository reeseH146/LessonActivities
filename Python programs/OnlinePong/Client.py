...
# Loading game objects - Ball/Paddle
# Paddle entity
class PaddleEntity:
...
# Main loop
while True:
    AvailableServer = select.select([ClientSocket], [], [], 0.1)
    ...
    if (CurrentTime - PreviousTime) > (1/60):
        ...
        try:
            ...
            try:
                print(5)
                ServerData = AvailableServer.recv(1024).decode("utf-8") # Polls for incoming data 
                print(ServerData)
                AvailableServer.send(f"Client {ClientNumber} connected".encode("utf-8"))
