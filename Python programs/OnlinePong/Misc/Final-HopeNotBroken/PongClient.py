...
while GameOn:
    ...
    if (CurrentTime - PreviousTime) > (1):
        ...
        try:
            ...
            if ClientNumber == 1:
                print(4)
                ClientSocket.sendall(f"{Player1.Position[1]}".encode("utf-8"))
                print(5)
            else:
                if ClientSocket.recv(128).decode("utf-8") == "Ready":
                    print(6)
                    ClientSocket.sendall(f"{Player2.Position[1]}".encode("utf-8"))
                    print(7)
            print(8)
